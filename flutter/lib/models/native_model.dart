...
class PlatformFFI {
  ...
  AlphadeskImpl get ffiBind => _ffiBind;
  ...
  void _startListenEvent(AlphadeskImpl AlphadeskImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = AlphadeskImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
